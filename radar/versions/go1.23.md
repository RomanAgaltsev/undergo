...
order is not guaranteed by the language spec". The sealed explanation for
`layout/01-struct-padding` opens with "Go lays a struct out in declaration
...
has to attribute the ordering to the implementation. The five measured answers
are unaffected; the explanation needs one correction.
